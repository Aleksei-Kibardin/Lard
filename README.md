...
### ПРОБЛЕМЫ:
```
тк я хотел постараться использовать vuetify по максимуму при попытке изсменить цвет текста менялся весь бек граунд поэтому было бы логичнее не использовать чипы так как их использовал я
